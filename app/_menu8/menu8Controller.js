...
(function(){
	angular.module('intechApp').controller('menu8Controller',[
		'$scope',
		'$http',
		'consts',
		'msgs',
	...
	function menu8Controller($scope, $http, consts, msgs, $compile, $interval, $location, lodash, trocaDados, querysql, auth, Upload, $timeout, $rootScope, generalfunc, $uibModal, getsetmodbus, $window, Excel, $state) {
		var vm = this;
		const url = trocaDados.getapiUrl()
		const registroModbusPorComponente = 2;
		var MENU_NAVEGACAO = ''
		const CONTROLLER = 'menu8Ctrl'
			...
			<display-gauge titulo="${area}" unidade="${unidade}" valor="${varModbusRead}" fator="${fator}"
				...
				vm.querySistema= function(id){
					...
					querysql.queryMysqlGetPost(cmdSql).then(function(response){
						...
						//============================================================================================
						MENU_NAVEGACAO = vm.sistemas[0].menu8_nome
						//============================================================================================
						//no caso de inserir outro menu adicionar aqui!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
						//============================================================================================
