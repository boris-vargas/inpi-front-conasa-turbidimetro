...
(function(){
	angular.module('intechApp').controller('menu6Controller',[
		'$scope',
		'$http',
		'consts',
		'msgs',
	...
	function menu6Controller($scope, $http, consts, msgs, $compile, $interval, $location, lodash, trocaDados, querysql, auth, Upload, $timeout, $rootScope, generalfunc, $uibModal, getsetmodbus, $window, Excel, $state) {
		var vm = this;
		const url = trocaDados.getapiUrl()
		const registroModbusPorComponente = 2;
		var MENU_NAVEGACAO = ''
		const CONTROLLER = 'menu6Ctrl'
			...
			<display-gauge titulo="${area}" unidade="${unidade}" valor="${varModbusRead}" fator="${fator}"
				...
				vm.querySistema= function(id){
					...
					querysql.queryMysqlGetPost(cmdSql).then(function(response){
						...
						//============================================================================================
						MENU_NAVEGACAO = vm.sistemas[0].menu6_nome
						//============================================================================================
						//no caso de inserir outro menu adicionar aqui!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
						//============================================================================================
				...
				vm.exportToPdfAlms = function(idElemento){
					html2canvas(document.getElementById(idElemento), {
						onrendered: function (canvas) {
							...
							var docDefinition = {
								pageOrientation: 'landscape',
								header: [{ text: '  relatórios', color: '#808080', alignment: 'center', fontSize: 22, margin: [0,10]}],
